...
bool HttpServer::listen(quint16 port) {
    ...
    // instead (see installWebSocketUpgrade()).  QAbstractHttpServer::bind()
    // returns void (it adopts the already-listening QTcpServer validated above)
    // and *reparents* the QTcpServer under itself.  Relinquish our unique_ptr so
    // the server isn't double-freed at shutdown (http_.reset() deletes the
    // adopted QTcpServer; without this, tcp_.reset() would delete it a second
    // time → heap corruption / __fastfail 0xC0000409).  release() hands back the
    // raw pointer we still use to read the bound port.
    QTcpServer* bound = tcp_.release();
    http_->bind(bound);
    port_ = bound->serverPort();
...
void HttpServer::installRoutes() {
    ...
    // responder to an lvalue reference) in Qt 6.8.  Through 6.7 it is a single
    // handler taking a QHttpServerResponder&& rvalue (MissingHandler typedef).
#if QT_VERSION >= QT_VERSION_CHECK(6, 8, 0)
