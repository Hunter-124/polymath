...
- **Agent toolset** — 31 tools: web search/fetch, image analysis, document & lab-report drafting,
  local document search (RAG), **computer use (see screen + mouse/keyboard)**, **live camera vision
  Q&A** (ask about a camera's current view), printing, shopping lists, reminders/tasks,
  camera/who's-home, Chrome automation, and the **instrument + lab-session** tools.
...
src/agent/       tool registry + 31 tools (web, docs, print, shopping, home, memory, browser, lab/instrument, doc-RAG, computer-use, camera-vision-Q&A)
